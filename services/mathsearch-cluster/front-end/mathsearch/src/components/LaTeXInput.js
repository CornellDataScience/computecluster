...
function LaTeXInput() {
  ...
  /** Function to upload PDF and LaTeX image to local API */
  const uploadRequest = async (file, text) => {
    try {
      // Render LaTeX to image blob
      const imageBlob = await renderLatexToBlob(text);
  ...
  type: file.type,
      ...
      const imageFile = new File([imageBlob], `${uuid}_image.png`, { type: 'image/png' });
      imageFormData.append('file', imageFile);
      imageFormData.append('uuid', uuid);
      imageFormData.append('type', 'image');

      // Upload PDF
      const pdfResponse = await fetch(`${API_BASE_URL}/upload`, {
        method: 'POST',
        body: pdfFormData,
      });
      ...
      console.log('Successfully uploaded PDF:', pdfResult);
      ...
      console.log('Successfully uploaded image:', imageResult);

    } catch (error) {
      console.error('Error uploading files:', error);
      throw error; // Re-throw to be caught by handleClick
    }
  };
